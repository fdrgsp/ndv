...
class NGFFWrapper(DataWrapper):
    ...
    def _parse_plate(self) -> None:
        ...
        for well_ref in plate_def.wells:
            ...
            if well_path not in self._zarr_group:
                logging.warning(f"Well {well_path} not found in plate")
                continue
            ...
            if not hasattr(well_metadata, "well"):
                logging.warning(f"Well {well_path} has no well metadata")
                continue
    ...
    def _parse_bioformats2raw(self) -> None:
        ...
        logging.debug("Parsing bioformats2raw multi-position layout")
        self._is_multiposition = True
    ...
    def _parse_single_image(self) -> None:
        ...
        logging.debug("Parsing single-position image")
        self._is_multiposition = False
